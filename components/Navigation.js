...
import { Search, Bell, Wind, LogOut, Heart, Clock, Mail } from 'lucide-react';
...
export const TopNavbar = ({
  ...
  isAdmin,
  onOpenNotifyModal,
  currentView, // 🆕 para saber qual aba está ativa
}) => {
    ...
    <nav className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-gray-200 h-16 flex items-center justify-between px-4 sm:px-6">
      ...
      <div className="flex-1 max-w-md mx-4 hidden sm:block relative">
        ...
        <input
          ...
          className="block w-full pl-10 pr-3 py-2 bg-gray-100 rounded-md text-sm focus:ring-2 focus:ring-blue-500 outline-none"
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="relative" ref={notifRef}>
          <button
            onClick={() => {
              ...
              if (!showNotif) onReadNotifications();
            }}
            className="p-2 text-slate-500 hover:text-blue-600 rounded-md relative"
          >
          ...
          {showNotif && (
            <div className="absolute right-0 mt-2 w-80 bg-white rounded-xl shadow-2xl border border-gray-100 overflow-hidden z-50">
              <div className="p-3 border-b bg-gray-50 text-sm font-bold text-gray-700">
                Notifications
              </div>
        ...
        {user && (
          <div
            ...
            className="h-8 w-8 rounded-full bg-gradient-to-tr from-blue-600 to-indigo-900 p-[2px] cursor-pointer relative hover:scale-105 transition"
          >
            <img
              src={user.avatar}
              className="rounded-full bg-white h-full w-full object-cover"
            />
            <div className="absolute -bottom-1 -right-1 bg-blue-600 text-white text-[8px] px-1.5 rounded-full font-bold border border-white">
              Lvl {Math.floor(user.stats.booksReadYear / 3) + 1}
            </div>
          </div>
        )}
    ...
    <div className="space-y-8">
      {user && (
        <div className="p-4 rounded-xl bg-slate-900 text-white shadow-lg relative overflow-hidden">
          ...
          <h3 className="text-xs font-medium text-slate-300 uppercase tracking-wider mb-1">
            Weekly Goal
          </h3>
          <div className="flex items-end gap-2 mb-3">
            <span className="text-3xl font-semibold">
              {user.weeklyGoal.current}
              <span className="text-slate-500 text-lg">
                /{user.weeklyGoal.target}
              </span>
            </span>
            <span className="text-sm text-slate-400 mb-1">books</span>
          </div>
          <div className="w-full bg-slate-700 h-1.5 rounded-full">
            <div
              ...
              style={{
                width: `${Math.min(
                  (user.weeklyGoal.current / user.weeklyGoal.target) * 100,
                  100
                )}%`,
              }}
            ></div>
      ...
      <div>
        ...
        <button
          onClick={() => setView('profile')}
          className="w-full flex items-center justify-between px-3 py-2 text-sm font-medium text-slate-600 hover:bg-gray-50 rounded-md"
        >
        ...
        <button
          onClick={() => setView('profile')}
          className="w-full flex items-center gap-3 px-3 py-2 text-sm font-medium text-slate-600 hover:bg-gray-50 rounded-md"
        >
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {genresList.map((g) => (
            <button
              ...
              className={`px-2.5 py-1 rounded-md border text-xs font-medium ${
                filter === g ? 'bg-blue-600 text-white' : 'bg-white'
              }`}
            >
              {g}
            </button>
